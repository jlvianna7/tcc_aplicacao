...
if v_tabelaCetic in ['E1', 'E2', 'G2', 'G3']:
...
if v_tabelaCetic in ['E1', 'E2', 'G2', 'G3', 'B18','B18a','B18b','B18c']:
    if v_tabelaCetic not in ['B18','B18a','B18b','B18c']:
        # Separa a pesquisa para gerar um dataframe separado por TOTAIS
        df_TOTAIS = monta_df.tabelaTOTAIS(v_pathCtiBr, v_ano, v_tabelaCetic)
...
if v_tabelaCetic in ['E1', 'E2', 'G2', 'G3']:
    df_tabTotais = pd.concat([df_TOTAIS2014, df_TOTAIS2015, df_TOTAIS2017, df_TOTAIS2019,
                            df_TOTAIS2021, df_TOTAIS2023, df_TOTAIS2024], ignore_index=True)
elif v_tabelaCetic in ['B18a','B18b','B18c']:
    df_tabTotais = pd.concat([df_TOTAIS2017, df_TOTAIS2019, df_TOTAIS2021, df_TOTAIS2023, 
                              df_TOTAIS2024], ignore_index=True)
elif v_tabelaCetic in ['H1Aa', 'H1Ab', 'H1Ac', 'H3', 'H3A', 'H3B', 'H4']:
    df_tabTotais = pd.concat([df_TOTAIS2019, df_TOTAIS2021, df_TOTAIS2023, 
                              df_TOTAIS2024], ignore_index=True)
else:
    df_tabTotais = pd.concat([df_TOTAIS2021, df_TOTAIS2023, df_TOTAIS2024], ignore_index=True) 
    
nomeTabela = ('usp_dsa.ft_' + v_tabelaCetic + '_TOTAIS').lower()
arquivoScriptis = v_pathScripts + '/CticBr_ScriptDML_SQLite.sql' 

#v_append_or_write = 'w'

with open(arquivoScriptis, v_append_or_write, encoding='utf-8') as f:
    sql = (
        f"\n/* POPULA TABELA {v_tabelaCetic} TOTAIS DA TABELA */\n\n"
    )
    f.write(sql)
    print (sql)
    for _, row in df_tabTotais.iterrows():
        ...
        sim = round(float(row[2]))
        sim = int(sim)

        sql = (
            f"INSERT INTO {nomeTabela} (ano_pesquisa, contexto, qtd_resposta_sim) "
            f"VALUES ('{ano_pesquisa}', '{contexto}', {sim}); \n"
        )
        f.write(sql)
    f.write("COMMIT; \n\n")
...
if v_tabelaCetic in ['E1', 'E2', 'G2', 'G3']:
    df_tabporte = pd.concat([df_porte2014, df_porte2015, df_porte2017, df_porte2019,
                            df_porte2021, df_porte2023, df_porte2024], ignore_index=True)
elif v_tabelaCetic in ['B18a','B18b','B18c']:
    df_tabporte = pd.concat([df_porte2017, df_porte2019,  df_porte2021, df_porte2023, df_porte2024], ignore_index=True)

elif v_tabelaCetic in ['H1Aa', 'H1Ab', 'H1Ac', 'H3', 'H3A', 'H3B', 'H4']:
    df_tabporte = pd.concat([df_porte2019,  df_porte2021, df_porte2023, df_porte2024], ignore_index=True)
else:
    df_tabporte = pd.concat([df_porte2021, df_porte2023, df_porte2024], ignore_index=True)
...
with open(arquivoScriptis, v_append_or_write, encoding='utf-8') as f:
    sql = (
        f"\n/* POPULA TABELA {v_tabelaCetic} POR PORTE DE EMPRESA */\n\n"
    )
    f.write(sql)
    ...
    for _, row in df_tabporte.iterrows():
        ...
        sql = (
            f"INSERT INTO {nomeTabela} (ano_pesquisa, id_porte, qtd_resposta_sim) "
            f"VALUES ('{ano_pesquisa}', '{contexto}', {sim});\n"
        )
        f.write(sql)
    f.write("COMMIT; \n\n")
...
if v_tabelaCetic in ['E1', 'E2', 'G2', 'G3']:
    df_tabmercado = pd.concat([df_mercado2014, df_mercado2015, df_mercado2017, df_mercado2019,
                              df_mercado2021, df_mercado2023, df_mercado2024], ignore_index=True)
elif v_tabelaCetic in ['B18a','B18b','B18c']:
    df_tabmercado = pd.concat([df_mercado2017, df_mercado2019, df_mercado2021, df_mercado2023, df_mercado2024], ignore_index=True)
elif v_tabelaCetic in ['H1Aa', 'H1Ab', 'H1Ac', 'H3', 'H3A', 'H3B', 'H4']:
    df_tabmercado = pd.concat([df_mercado2019, df_mercado2021, df_mercado2023, df_mercado2024], ignore_index=True)
else:
    df_tabmercado = pd.concat([df_mercado2021, df_mercado2023, df_mercado2024], ignore_index=True)
...
with open(arquivoScriptis, v_append_or_write, encoding='utf-8') as f:
    sql = (
        f"\n/* POPULA TABELA {v_tabelaCetic} POR MERCADO DE ATUAÇÃO */\n\n"
    )
    f.write(sql)

    for _, row in df_tabmercado.iterrows():
#        print(row['Ano_pesquisa'], " ... ", row['Contexto'])
        ano_pesquisa = row['Ano_pesquisa'].replace(
            "'", "''")  # Escapa aspas simples
        ...
        sql = (
            f"INSERT INTO {nomeTabela} (ano_pesquisa, id_mercado, qtd_resposta_sim) "
            f"VALUES ('{ano_pesquisa}', '{contexto}', {sim});\n"
        )
        f.write(sql)
    f.write("COMMIT; \n\n")
