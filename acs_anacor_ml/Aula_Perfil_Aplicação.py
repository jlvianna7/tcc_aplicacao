...
cma = pd.read_excel("valendo_contraposição_mudanca_area3.xlsx")
...
print(cma['var_1'].value_counts())
print(cma['var_2'].value_counts())

#%% Criando a tabela de contingência

tabela = pd.crosstab(cma["var_1"], cma["var_2"])
...
fig.write_html('assoc_cma_maturidade.html')
...
sns.scatterplot(data=chart_df_row, x='x_row', y='y_row', s=100, marker='*', color='black')
sns.scatterplot(data=chart_df_col, x='x_col', y='y_col', s=40, color='gray')
sns.despine(top=True, right=True, left=False, bottom=False)
plt.axhline(y=0, color='lightgrey', ls='--')
plt.axvline(x=0, color='lightgrey', ls='--')
plt.tick_params(size=2, labelsize=6)
#plt.suptitle("Análise de Correspondência Simples - Mapa Perceptual", fontsize=10)
#plt.title("Organização com ou sem CMO x Maturidade na condução de mudanças", fontsize=8)
...
coord_obs = cma.rename(columns={'cma':'var_row',
