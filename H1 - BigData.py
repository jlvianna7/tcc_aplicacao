...
#
# H1 - Evolução cronológoia de empresas que utilizaram plataformas ERP por Mercado de atuação

# %% Demonstra a prticipação percentual por Mercado de atuação

col1.subheader("Evolução da utilização de :yellow-background[Big Data] conforme Origem dos Dados")
col2.write(' ')
col3.write(' ')
...
sql = (
    f'SELECT DISTINCT dic.nm_questao_variavel "Origem dos dados" '
    f'FROM ft_ceticbr_mercado f, dm_dicionario_questoes_ceticbr dic '
    f'WHERE f.cd_variavel = dic.cd_questao_ceticbr '
    f'AND f.cd_variavel like "h1a%" '
)

bd = f_ConectaBD.conn
dfOrigBox = pd.read_sql(sql, bd)

v_origem = dfOrigBox['Origem dos dados'].value_counts().index
cbox_origem = col1.selectbox('Selecione a Origem dos dados do Big Data a pesquisar', v_origem)
...
dfM = dfM[dfM["Origem dos dados"] == cbox_origem]
...
###############################################################################################
#
# H1 - Evolução cronológoia de empresas que utilizaram plataformas Big Data por Porte da empresa
